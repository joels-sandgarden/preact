...
#### Troubleshooting: `no-restricted-imports` in frontend code

Grafana uses ESLint rules to restrict certain imports in specific parts of the repository. When you hit a `no-restricted-imports` error, follow the rule message first. The message typically points to the supported alternative.

Some restrictions are specific to dataviz and TableNG code:

- Do not import `cx` from `@emotion/css`. Use `clsx` instead, and compose only strings.
- In `packages/grafana-ui/src/components/Table/TableNG/**`, do not import `getFieldDisplayName` from `@grafana/data`. Use the local `getDisplayName` helper from the TableNG utils instead.
