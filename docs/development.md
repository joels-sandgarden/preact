...
- Use short, descriptive branch names that match the change.
...
- Publish release versions from that validated state.

## Common tasks

- `npm run dev` watches the core package during local development.
- `npm run dev:hooks` watches the hooks package.
- `npm run dev:compat` watches the compat package.
...
- `npm run test:ts` runs the TypeScript checks.
