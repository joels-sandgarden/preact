...
This document describes the core runtime that turns `vnode`s into `DOM`, updates `components`, and handles `context`, `cloning`, `hooks`, and `errors`.

## Source map

| Concept | Source files | What the runtime does |
| --- | --- | --- |
| Vnode creation | `src/create-element.js`, `jsx-runtime/src/index.js` | Builds normalized `vnode`s from `createElement`, `Fragment`, and JSX runtime calls. |
